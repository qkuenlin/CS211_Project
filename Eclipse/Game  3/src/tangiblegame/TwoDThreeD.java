...
public class TwoDThreeD {
	...
	private double[][] solveExtrinsicMatrix(List<PVector> points2D) {
		...
		for(int i=0;i<4;i++){
			...
			float [] point = {points2D.get(i).x,points2D.get(i).y,1};
			float[] result = Mat.multiply(invK, point);
			projectedCorners[i][0]= result[0];
			projectedCorners[i][1]=	result[1];
			projectedCorners[i][2]= result[2];
		}
